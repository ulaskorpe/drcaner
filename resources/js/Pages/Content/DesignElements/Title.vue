<template>
    ...
    <div>
            ...
            <div class="col-md-9">
                <div class="d-flex gap-4">
                    <div class="d-flex flex align-items-center">
                        <RadioButton v-model="elemDetails.data.tag" inputId="title-tag-h1" name="tag" value="h1" />
                        <label for="title-tag-h1" class="ms-2">H1</label>
                    </div>
                    <div class="d-flex flex align-items-center">
        ...
        <div class="row align-items-center">
            <label class="col-md-3 fw-bold text-uppercase">TAGLINE</label>
            <div class="col-md-9">
                <InputText class="w-100" v-model="elemDetails.data.tagline"/>
            </div>
        </div>
        ...
        <div class="row align-items-center">
            <label class="col-md-3 fw-bold text-uppercase">BG BASE COLOR</label>
            <div class="col-md-9">
                <InputSwitch v-model="elemDetails.data.bgBaseColor"/>
            </div>
        </div>

        <div v-if="!elemDetails.data.bgBaseColor">
            <Divider />
        ...
        <div class="row align-items-center">
            <label class="col-md-3 fw-bold text-uppercase">THEME STYLE</label>
            <div class="col-md-9">
                <Dropdown v-model="elemDetails.data.titleStyle" showClear :options="['title-style-one','title-style-two','title-style-three','title-style-four','title-style-five']" class="w-100" />
            </div>
        </div>

        <div v-if="elemDetails.data.titleStyle">
            <Divider />

            <div class="row align-items-center">
                <label class="col-md-3 fw-bold text-uppercase">THEME STYLE SEP. COLOR</label>
                <div class="col-md-9">
                    <Dropdown v-model="elemDetails.data.titleSepBgColor" showClear filter :options="beforeAfterBgColors" optionValue="value" optionLabel="label" class="w-100" />
                </div>
            </div>
        </div>
...
import {textColors,displaySizes,fontWeights, beforeAfterBgColors, slugify} from "./design-elements-data.js";
...
const prepareElement = async () => {
    ...
    let promise = new Promise(function(resolve, reject) {
        ...
        if(elemDetails.value.data.bgBaseColor){
            elemDiv.classList.add('text-light');
            elemDiv.classList.add('mix-blend-mode-difference');
        } else if(elemDetails.value.data.color && elemDetails.value.data.color != ""){
        ...
        if(elemDetails.value.data.titleStyle && elemDetails.value.data.titleStyle != ""){
            elemDiv.classList.add(elemDetails.value.data.titleStyle);
            if(elemDetails.value.data.titleSepBgColor && elemDetails.value.data.titleSepBgColor != ""){
                elemDiv.classList.add(elemDetails.value.data.titleSepBgColor);
            }
        }
        ...
        elemDiv.innerHTML = elemDetails.value.data.text+" ";
        elemDiv.dataset.text = elemDiv.innerText;

        if(elemDetails.value.data.tagline && elemDetails.value.data.tagline != ""){
            const titleTagLine = document.createElement("span");
            titleTagLine.classList.add("tagline");
            titleTagLine.innerHTML = elemDetails.value.data.tagline;
            elemDiv.appendChild(titleTagLine);
        }
