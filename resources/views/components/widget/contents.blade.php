...
<div @class($elemDivClasses)>
    ...
    @switch($widgetData['type'])
        ...
        <div @class($elemClasses)>
            ...
            @foreach ($contents as $key => $content)
            <div class="accordion-item bg-black border-0 rounded">
                <h3 class="accordion-header border-bottom font-body">
                  <button class="accordion-button rounded fw-semibold bg-black text-white text-uppercase collapsed" type="button" data-bs-toggle="collapse" data-bs-target="#acc-collapse-{{$content->id}}" aria-expanded="true" aria-controls="acc-collapse-{{$content->id}}">
                    {!! $content->name !!}
                  </button>
                </h3>
